...
import { verifyWarehouseSession } from "./auth.js";

export async function assertWarehouseAuth(req) {
  const bearer = readBearer(req);
  const admin = await verifyWarehouseSession(bearer);
  if (admin) {
    return admin;
  }
  ...
  return { email: "token-admin", role: "admin" };
}

export async function warehouseOverview() {
  const sql = db();
  const [categories, products, variants, accounts, orders, logs, admins, drafts] = await Promise.all([
    sql`
      select c.*,
    ...
    `,
    sql`
      select id, telegram_id, parsed, status, created_at, reviewed_at
  ...
  return { categories, products, variants, accounts, orders, logs, admins, drafts };
}
...
export async function deleteProduct({ product_id }) {
  const id = requiredInt(product_id, "product_id");
  const rows = await db()`
    update products
    set is_active = false
    where id = ${id}
    returning *
  `;
  await logWarehouse("warehouse.product.deleted", { product_id: id });
  return rows[0];
}

export async function deleteVariant({ variant_id }) {
  const id = requiredInt(variant_id, "variant_id");
  const rows = await db()`
    ...
    returning *
  `;
  await logWarehouse("warehouse.variant.deleted", { variant_id: id });
  return rows[0];
}

export async function deleteAccount({ account_id }) {
  const id = requiredInt(account_id, "account_id");
  const rows = await db()`
    update accounts
    set status = 'disabled',
        note = case when note = '' then 'Đã xóa khỏi kho' else note || ' | Đã xóa khỏi kho' end
    where id = ${id}
    returning id, status, note
  `;
  await logWarehouse("warehouse.account.deleted", { account_id: id });
  return rows[0];
}

export async function deleteImportDraft({ draft_id }) {
  const id = requiredInt(draft_id, "draft_id");
  const rows = await db()`
    ...
    returning *
  `;
  await logWarehouse("warehouse.import_draft.deleted", { draft_id: id });
  return rows[0];
}
