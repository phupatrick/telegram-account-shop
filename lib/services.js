...
export async function startSmartImportSession(telegramId) {
  const rows = await db()`
    insert into warehouse_intake_sessions (telegram_id, is_active, updated_at)
    values (${String(telegramId)}, true, now())
    on conflict (telegram_id)
...
export async function hasActiveSmartImportSession(telegramId) {
  const rows = await db()`
    select *
...
export async function stopSmartImportSession(telegramId) {
  await db()`
    update warehouse_intake_sessions
    set is_active = false,
        updated_at = now()
    where telegram_id = ${String(telegramId)}
  `;
...
export async function approveSmartImportDraft(draftId, telegramId) {
  ...
  return sql.begin(async (tx) => {
    ...
    `;
    await tx`
      insert into warehouse_import_training_examples (draft_id, telegram_id, raw_text, parsed)
      values (${draft.id}, ${String(telegramId)}, ${draft.raw_text}, ${JSON.stringify(parsed)}::jsonb)
    `;
...
function classifyInventoryText(rawText, catalog) {
  ...
  const lines = prepareInventoryLines(rawText);
  ...
  for (const row of catalog) {
    ...
    if (row.variant_id) {
      map.get(row.product_id).variants.push({
        ...
        mailType: normalizeText(row.mail_type || ""),
...
function findCatalogMatch(line, products, context) {
  ...
  for (const product of products) {
    ...
    for (const variant of variants) {
      const variantScore = variant ? scoreMatch(normalized, variant.normalizedName, variant.tokens) + inferVariantScore(normalized, variant) : 0;
...
function prepareInventoryLines(rawText) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (isEmailOnlyLine(line) && next && looksLikePasswordOnlyLine(next)) {
      prepared.push(`${line}|${next}`);
      i += 1;
      continue;
    }
    prepared.push(line);
...
function inferVariantScore(normalizedLine, variant) {
  let score = 0;
  const warranty = Number(variant.warrantyDays) || 0;
  const mentionsWarranty = /\b(bh|bao hanh|warranty|cap)\b/.test(normalizedLine);
  const requestedDays = Number(normalizedLine.match(/\b(0|7|10|14|28|30|60|90)\b/)?.[1] || NaN);

  if (Number.isFinite(requestedDays) && warranty === requestedDays && mentionsWarranty) {
    score += 8;
  }

  if (warranty === 0 && /\b(khong|ko|no|0)\b.*\b(bh|bao hanh|warranty)\b|\b(bh|bao hanh|warranty)\b.*\b(khong|ko|no|0)\b/.test(normalizedLine)) {
    score += 7;
  }

  if (/\b(gmail|hotmail|outlook)\b/.test(normalizedLine) && /\b(gmail|outlook|hotmail)\b/.test(variant.mailType)) {
    score += 5;
  }

  if (/\brandom\b/.test(normalizedLine) && /\brandom\b/.test(variant.mailType)) {
    score += 4;
  }
...
function isEmailOnlyLine(line) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(line.trim());
}

function looksLikePasswordOnlyLine(line) {
  const normalized = normalizeText(line);
  return !line.includes("@") && !/\s/.test(line.trim()) && (line.trim().length >= 4 || /\b(pass|pwd|password)\b/.test(normalized));
}
...
function normalizeText(value) {
  return String(value || "")
    ...
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/\bngya?f\b/g, "ngay")
    .replace(/\bngafy\b/g, "ngay")
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
}
